            ...
            that.$confirm('此操作将删除该订单，是否继续?', '提示', {
                ...
                that.ajax.post('/xinda-api/business-order/del',that.qs.stringify(
                    ...
                    function(data){
                        ...
                        // if(that.orderArr.length==0){that.fenye=0;return;}
                ...
                function(data){  
                    ...
                    if(panchu==1){
                        for(var i in data.data.data){
                            ...
                            // console.log(data.data.data[i].businessNo);
                        ...
                        // var xunhuan=that.orderArr.slice(that.orderArr.length-limited,that.orderArr.length);
                        // // console.log(xunhuan);
                        // for(var i in xunhuan){
                        //     that.ajax.post('/xinda-api/service-order/grid',that.qs.stringify(
                        //         {'businessNo':xunhuan[i].businessNo}
                        //     )).then(
                        //         function(data){
                        //             that.changeDate(data.data.data);
                        //             that.orderArr1.push(data.data.data); 
